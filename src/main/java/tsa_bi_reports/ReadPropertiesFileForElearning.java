...
public class ReadPropertiesFileForElearning implements Callable{
	public Object onCall(MuleEventContext eventContext) throws Exception {
		...
		prop.load(input);
		int queryCount = Integer.parseInt(prop.getProperty("tsa.query.iterate.count").toString());
		...
		for(int i=1; i<=queryCount; i++){
			...
			if(i == 13){
				muleClient.dispatch("vm://TSADB2", localMsg);
			}
